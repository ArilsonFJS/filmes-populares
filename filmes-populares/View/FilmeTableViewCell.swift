...
//  filmes-populares
//
//  Created by Arilson Silva on 04/03/23.
//
...
class FilmeTableViewCell: UITableViewCell {
    ...
    func configureCell(filme: Filme) {
        setConstraints()
        titleLabel.text = filme.title
        releaseDateLabel.text = "Lançamento: \(filme.releaseDate.formatDate())"
        guard let imageURL = filme.image else { return }
